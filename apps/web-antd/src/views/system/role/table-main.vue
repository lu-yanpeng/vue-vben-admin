...
interface FormFields {
  ...
  name?: string;
  desc?: string;
  create_time?: [dayjs.Dayjs, dayjs.Dayjs];
  update_time?: [dayjs.Dayjs, dayjs.Dayjs];
  is_default_role?: boolean;
  db_or_query_?: boolean;
}
      ...
      ajax: {
        ...
        query: async ({ page }, formValues: Partial<FormFields>) => {
          ...
          for (const itemKey in formValues) {
            const key = itemKey as keyof FormFields;
            const value = formValues[key] as any;
            if (value !== undefined) {
              switch (typeof value) {
                case 'boolean': {
                  queryData[key] = ['==', value] as any;
                  break;
                }
                case 'number': {
                  queryData[key] = ['==', value] as any;
                ...
                case 'string': {
                  queryData[key] = ['like', value] as any;
                  break;
                }
              }
            }
          ...
          if (
            !isEmpty(queryData) && // db_or_query_是默认勾选的，如果只有这个参数，就不要传递
            !(
